...
public class BeanMethodValid extends BaseValid implements IValid {
	...
	public BeanMethodValid(String proName, String desc, String bmethod) {
		...
		if(beanAndMethod.length != 2){
			throw new RuntimeException("bmethod的格式必须为xx.yy  其中xx指bean名字，yy指方法名 bmethod : " + bmethod);
		}
		...
		try {
			validMethod = bean.getClass().getMethod(beanAndMethod[1], new Class[]{Object.class, Object.class});
		} catch (Exception e) {
			throw new RuntimeException("指定bean不存在该方法 ：public boolean " + beanAndMethod[1]+"(Object srcObj,Object value)", e);
		}
		Class clazz = validMethod.getReturnType();
		if(clazz == null || !clazz.toString().equals("boolean")){
			throw new RuntimeException("指定bean不存在该方法 ："+ bean.getClass() +" public boolean " + beanAndMethod[1]+"(Object srcObj,Object value)");

		}
		methodStr = bean.getClass().getName() + ". public boolean " + beanAndMethod[1]+"(Object srcObj,Object value)";
	}
